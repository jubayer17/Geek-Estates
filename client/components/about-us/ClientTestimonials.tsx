      ...
      <div className="container mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="lg:hidden space-y-10">
          <div className="space-y-4">
            <span className="text-[#E7C873] uppercase tracking-[0.2em] text-xs font-bold block">
              Client Voices
            </span>
          ...
          <div className="flex overflow-x-auto snap-x snap-mandatory gap-4 pb-6 -mx-6 px-6 scrollbar-hide">
            {testimonials.map((item) => (
              <div
                key={item.id}
                className="min-w-[85vw] sm:min-w-[60vw] snap-center bg-gray-50 p-8 border-l-2 border-[#E7C873] flex flex-col relative"
              >
                <div className="absolute top-6 right-6 opacity-10 text-[#1A1A1A]">
                  <Quote size={40} />
                </div>

                <div className="flex gap-1 mb-6">
                  {[...Array(item.rating)].map((_, i) => (
                    <Star key={i} className="w-4 h-4 fill-[#E7C873] text-[#E7C873]" />
                  ))}
                </div>

                <blockquote className="text-xl font-serif italic text-[#1A1A1A] mb-8 leading-relaxed flex-grow">
                  &quot;{item.content}&quot;
                </blockquote>

                <div className="mt-auto">
                  <h4 className="text-lg font-serif text-[#1A1A1A] font-medium">{item.name}</h4>
                  <p className="text-xs text-gray-500 uppercase tracking-wider mt-1">{item.role}</p>
                  <p className="text-xs text-[#E7C873] font-bold uppercase tracking-widest mt-2">{item.company}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Desktop View (Grid) */}
        <div className="hidden lg:grid lg:grid-cols-12 gap-12 lg:gap-20 items-start">
