...
export default function CareerPerksForm() {
  ...
  const fetchData = async () => {
    try {
      const [pageRes, perksRes] = await Promise.all([
        ...
        fetch('https://geek-estates.onrender.com/career/perks'),
      ]);
  ...
  const handlePageUpdate = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch('https://geek-estates.onrender.com/career/page', {
        method: 'PATCH',
  ...
  const handleAddPerk = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const res = await fetch('https://geek-estates.onrender.com/career/perks', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleDeletePerk = async (id: string) => {
    ...
    try {
      await fetch(`https://geek-estates.onrender.com/career/perks/${id}`, { method: 'DELETE' });
